export async function openDriveViaWebRtcInvite(parsed, relayUrl, libs) {
  ...
  if (!parsed.nativeInvite) throw new Error('Invite is missing native invite context')
...
function waitForChannelOpen(channel, timeoutMs) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('Timed out waiting for direct WebRTC channel')), timeoutMs)
    channel.onopen = () => {
      clearTimeout(timer)
      resolve()
    }
