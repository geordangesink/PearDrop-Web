...
test('web app downloads from bare-backed native uploader', { timeout: 60000 }, async (t) => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'peardrops-web-e2e-'))
  ...
  try {
    ...
    const state = JSON.parse(stateLine.slice(stateLine.indexOf('STATE:') + 'STATE:'.length))

    console.log('[e2e] start vite server')
    webProc = spawn(
      process.execPath,
      ['node_modules/vite/bin/vite.js', '--host', '127.0.0.1', '--port', String(WEB_PORT), '--strictPort'],
      {
        cwd: process.cwd(),
        env: process.env,
        stdio: ['ignore', 'pipe', 'pipe']
    ...
    await page.waitForFunction(
      () => {
        ...
        return el && (/Connected\./.test(el.textContent || '') || /Join failed:/.test(el.textContent || ''))
      },
      { timeout: 20000 }
    )

    const statusText = await page.textContent('#status')
    assert.match(String(statusText || ''), /Connected\./, `Expected Connected status, got: ${statusText}`)
    ...
    if (page) {
      try {
        const debug = await page.evaluate(() => ({
          ...
          hasTestOpenDrive: typeof globalThis.__PEARDROPS_TEST_OPEN_DRIVE__ === 'function',
