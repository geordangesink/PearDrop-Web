...
async function main() {
  ...
  const baseDir = String(config.baseDir || `/tmp/peardrops-bare-uploader-${Date.now()}-${Math.random().toString(16).slice(2)}`)
                ...
                socket.write(
                  b4a.from(
                    JSON.stringify({
                      id,
                      ok: true,
                      dataBase64: rawFile ? b4a.toString(rawFile, 'base64') : ''
                    }) + '\n'
                  )
                )
              }
